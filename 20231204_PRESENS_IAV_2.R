Data_CA <- read.csv("20231209_IAV_PRESENS_CA_2.csv")
...
data <- select(Data_CA, collection_date, IAV, IAV_Nor, IAV_day)#Data_CAファイル名を変える

#Moving average; 7-day, 5-day, 0-day
data_2_7 <- select(Data_CA, collection_date, IAV_central, IAV_Nor_Central, IAV_day, IAV_cas_7_day)
data_2_5 <- select(Data_CA, collection_date, IAV_5, IAV_Nor_5, IAV_day, IAV_cas_7_day)
data_2_0 <- select(Data_CA, collection_date, IAV_0, IAV_Nor_0, IAV_day, IAV_cas_7_day)
...
data_WBE <- select(data_lead, collection_date, IAV) #ここのdataファイル名と数字を変える
colnames(data_WBE) <- c("collection_date", "IAV")
data_WBE <- data_WBE %>% drop_na()


#Pick up row that contains IAV concentration data
IAV_data_row <- data.frame(true = which(!is.na(data_lead$IAV)))#ここのdataファイル名と数字を変える
...
print(mcmc_CA, pars = c("A", "a", "b", "sigma"), probe = c(0.025, 0.50, 0.975))
...
IAV_data_row <- data.frame(true = which(!is.na(data_lead$IAV)))
nrow(Data_CA)
...
data_result <- data.frame(A = summary_2_A, B = summary_2_B, C = summary_2_C)
colnames(data_result) <- c("2.5%", "50%", "97.5%")
write.csv(x = data_result, file = "C:/2023_R/PRESENS_prediction_WBE_7_final_day.csv")
